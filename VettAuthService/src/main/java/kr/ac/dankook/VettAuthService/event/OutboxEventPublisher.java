...
public class OutboxEventPublisher {
    ...
    public void publishOutboxEvent(String id, String eventTopic, String partitionKey, String payload){

        if (outboxCacheService.checkIsAlreadyPublish(id)) return;

        CompletableFuture<SendResult<String, String>> future = kafkaTemplate.send(eventTopic, partitionKey, payload);
        future.whenComplete((result,ex) -> {
            if (ex == null){
                outboxCacheService.setOutboxId(id,OUTBOX_TTL);
                outboxService.convertOutboxStatus(id, OutboxStatus.PUBLISHED);
                log.info("{}, CLASS={}, METHOD={}, EVENT_ID={}, TOPIC={}, PAYLOAD={}",
                        LogMessage.PUBLISH_OUTBOX_EVENT, "OutboxEventPublisher", "publishOutboxEvent",
                        id, eventTopic, payload);
            }else{
                outboxCacheService.deleteOutboxId(id);
                outboxService.convertOutboxStatus(id, OutboxStatus.FAILED);
                log.error("{}, CLASS={}, METHOD={}, EVENT_ID={}, TOPIC={}, PAYLOAD={}, ERROR={}",
                        LogMessage.FAILED_PUBLISH_OUTBOX_EVENT, "OutboxEventPublisher", "publishOutboxEvent",
                        id, eventTopic, payload, ex.getMessage());
            }
        });
    }
}
